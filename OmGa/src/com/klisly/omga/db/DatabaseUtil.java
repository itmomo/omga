...
import com.klisly.omga.db.DBHelper.FavTable;
...
public class DatabaseUtil {
	 private static final String TAG="DatabaseUtil";
	    ...
	    public void deleteFav(Qiushi qy){
	    	Cursor cursor=null;
	    ...
	    public boolean isLoved(Qiushi qy){
	    	Cursor cursor = null;
	    	String where = FavTable.USER_ID+" = '"+MyApplication.getInstance().getCurrentUser().getObjectId()
	    			+"' AND "+FavTable.OBJECT_ID+" = '"+qy.getObjectId()+"'";
	    	cursor=dbHelper.query(DBHelper.TABLE_NAME, null, where, null, null, null, null);
	    ...
	    public long insertFav(Qiushi qy){
	    	long uri = 0;
	    	Cursor cursor=null;
	    	String where = FavTable.USER_ID+" = '"+MyApplication.getInstance().getCurrentUser().getObjectId()
	    			+"' AND "+FavTable.OBJECT_ID+" = '"+qy.getObjectId()+"'";
...
//		    	cv.put(FavTable.IS_LOVE, qy.getL==true?1:0);
...
//	    public int deleteFav(Qiushi qy){
//	    	int row = 0;
...
//	    }
	     ...
	     * @param lists
	     */
	    public List<Qiushi> setFav(List<Qiushi> lists) {
	        Cursor cursor=null;
	        if(lists != null && lists.size() > 0) {
	            for(Iterator iterator=lists.iterator(); iterator.hasNext();) {
	            	Qiushi content=(Qiushi)iterator.next();
...
//	                LogUtils.i(TAG,content.getMyFav()+".."+content.getMyLove());
	     ...
	     * @param lists
	     */
	    public List<Qiushi> setFavInFav(List<Qiushi> lists) {
	        Cursor cursor=null;
	        if(lists != null && lists.size() > 0) {
	            for(Iterator iterator=lists.iterator(); iterator.hasNext();) {
	            	Qiushi content=(Qiushi)iterator.next();
...
//	                LogUtils.i(TAG,content.getMyFav()+".."+content.getMyLove());
	    ...
	    public ArrayList<Qiushi> queryFav() {
	        ArrayList<Qiushi> contents=null;
